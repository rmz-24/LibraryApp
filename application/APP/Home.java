...
public class Home {
	...
	private void loadRecentBooks() {
	    ...
	    final String query = "SELECT BOOKNAME, COPIESNBR FROM (SELECT BOOKNAME, COPIESNBR, ROW_NUMBER() OVER (ORDER BY BOOKID  DESC) AS rn FROM BOOKSLIST) WHERE rn <= 10";
	    ...
	    try (PreparedStatement stmtn = conn.prepareStatement(query)) {
	        ...
	        while (rs.next() && bookCount < bookNameLabels.length) {
	            ...
	            String bookcopies = "  " + rs.getInt("COPIESNBR");
